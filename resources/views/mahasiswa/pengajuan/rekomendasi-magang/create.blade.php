<x-app-layout>
    <x-slot name="title">Ajukan Rekomendasi Magang</x-slot>
    <div class="mb-4 flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('mahasiswa.riwayat.index') }}" class="hover:text-brand-600">Riwayat</a>
        <span>/</span><span class="text-gray-700">Rekomendasi Magang</span>
    </div>
    <div class="max-w-2xl mx-auto">
        <div class="rounded-xl border bg-white p-6 shadow-sm">
            <h2 class="mb-5 text-base font-semibold text-slate-800">Ajukan Surat Rekomendasi Magang</h2>
            <form method="POST" action="{{ route('mahasiswa.pengajuan.rekomendasi-magang.store') }}"
