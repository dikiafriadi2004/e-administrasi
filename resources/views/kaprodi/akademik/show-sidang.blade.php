﻿<x-app-layout>
    <x-slot name="title">Keputusan Sidang Skripsi</x-slot>

    <div class="mb-4 flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('kaprodi.akademik.index') }}" class="hover:text-brand-600">Antrian Akademik</a>
        <x-icon name="chevron-right" class="h-4 w-4 text-slate-300" /><span class="text-gray-700">Sidang Skripsi</span>
    </div>
    ...
    <div class="mx-auto max-w-3xl space-y-5"
         x-data="{
             modalSetujui: false,
             modalTolak: false,
             penguji1: '{{ $seminarSebelumnya?->dosen_penguji_id ?? '' }}',
             penguji1Nama: '{{ addslashes($seminarSebelumnya?->dosenPenguji?->nama ?? '') }}',
             penguji2: '{{ $seminarSebelumnya?->dosen_penguji_2_id ?? '' }}',
             penguji2Nama: '{{ addslashes($seminarSebelumnya?->dosenPenguji2?->nama ?? '') }}'
         }">
        ...
        <div class="rounded-xl border bg-white p-6 shadow-sm">
            <div class="mb-4 flex items-start justify-between gap-3">
                <div>
                    <h2 class="text-base font-semibold text-slate-800">Pengajuan Sidang Skripsi</h2>
                    <p class="mt-0.5 text-xs text-gray-400">Diajukan {{ $pengajuan->created_at->format('d M Y, H:i') }}</p>
                </div>
            ...
            <dl class="space-y-3 text-sm">
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <dd class="col-span-2 text-gray-800">{{ $pengajuan->mahasiswa->user->name }}
                        <span class="text-gray-400">({{ $pengajuan->mahasiswa->nim }})</span></dd>
                </div>
                <div class="grid grid-cols-3 gap-2">
                    <dt class="font-medium text-gray-500">Judul Skripsi</dt>
                    <dd class="col-span-2 leading-relaxed text-gray-800">{{ $pengajuan->pengajuanJudul?->judul ?? '—' }}</dd>
                </div>
                <div class="grid grid-cols-3 gap-2">
                    <dt class="font-medium text-gray-500">Dosen Pembimbing</dt>
                    <dd class="col-span-2 text-gray-800">{{ $pengajuan->pengajuanJudul?->dosenPembimbing?->nama ?? '—' }}</dd>
                </div>
                @if ($pengajuan->pengajuanJudul?->dosenPembimbing2)
                    <div class="grid grid-cols-3 gap-2">
                        <dt class="font-medium text-gray-500">Pembimbing 2</dt>
                        <dd class="col-span-2 text-gray-800">{{ $pengajuan->pengajuanJudul->dosenPembimbing2->nama }}</dd>
                    </div>
                @endif
                <div class="grid grid-cols-3 gap-2">
                    <dt class="font-medium text-gray-500">Rencana Tanggal</dt>
                    <dd class="col-span-2 text-gray-800">{{ $pengajuan->data_form['tanggal_rencana'] ?? '—' }}</dd>
                </div>
                <div class="grid grid-cols-3 gap-2">
                    <dt class="font-medium text-gray-500">Waktu</dt>
                    <dd class="col-span-2 text-gray-800">{{ $pengajuan->data_form['waktu_rencana'] ?? '—' }}</dd>
                </div>
                <div class="grid grid-cols-3 gap-2">
                    <dt class="font-medium text-gray-500">Tempat</dt>
                    <dd class="col-span-2 text-gray-800">{{ $pengajuan->data_form['tempat'] ?? '—' }}</dd>
                </div>
                @if ($pengajuan->dosenPenguji)
                    <div class="grid grid-cols-3 gap-2">
                        <dt class="font-medium text-gray-500">Penguji 1 Ditetapkan</dt>
                        <dd class="col-span-2 font-semibold text-brand-700">{{ $pengajuan->dosenPenguji->nama }}</dd>
                    </div>
                @endif
                @if ($pengajuan->dosenPenguji2)
                    <div class="grid grid-cols-3 gap-2">
                        <dt class="font-medium text-gray-500">Penguji 2 Ditetapkan</dt>
        ...
        @if ($pengajuan->status === 'diajukan')
            {{-- Info penguji dari seminar sebelumnya --}}
            @if ($seminarSebelumnya?->dosenPenguji)
                <div class="rounded-xl border border-brand-100 bg-brand-50 px-4 py-3 text-xs">
                    <p class="mb-1.5 font-semibold text-brand-700">
                        <x-icon name="info" class="h-3.5 w-3.5 inline mr-1" />
                        Penguji dari Seminar Proposal (sudah dipilih otomatis)
                    </p>
                    <p class="text-brand-600">Penguji I: <strong>{{ $seminarSebelumnya->dosenPenguji->nama }}</strong></p>
                    @if ($seminarSebelumnya->dosenPenguji2)
                        <p class="text-brand-600">Penguji II: <strong>{{ $seminarSebelumnya->dosenPenguji2->nama }}</strong></p>
                    @endif
                    <p class="mt-1 text-brand-500 italic">Anda dapat mengganti jika ada perubahan.</p>
                </div>
            @endif

            {{-- Tabel Rekomendasi Penguji --}}
            <div class="rounded-xl border bg-white p-6 shadow-sm">
                <h3 class="mb-1 text-sm font-semibold text-gray-700">Pilih Dosen Penguji</h3>
                <p class="mb-1 text-xs text-gray-400">Dosen pembimbing sudah di-exclude. <strong>Penguji 1 dan Penguji 2 wajib dipilih.</strong> Diurutkan dari beban pengujian terkecil.</p>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($dosenTerurut as $i => $dosen)
                                <tr class="{{ $i === 0 ? 'bg-green-50' : 'hover:bg-gray-50' }}">
                                    ...
                                    <td class="px-4 py-2">
                                        <p class="font-medium text-gray-800">{{ $dosen->nama }}</p>
                                        <p class="font-mono text-xs text-gray-400">{{ $dosen->nip }}</p>
                                    </td>
                                    <td class="px-4 py-2 text-center font-bold text-gray-700">{{ $dosen->jumlah_pengujian }}</td>
                                    <td class="px-4 py-2 text-center">
                                        <button type="button"
                                            ...
                                            <span x-text="penguji1 === '{{ $dosen->id }}' ? 'Dipilih P1' : 'P1'"></span>
                                        </button>
                                    </td>
                                    <td class="px-4 py-2 text-center">
                                        <button type="button"
                                                ...
                                                class="rounded-lg px-3 py-1 text-xs font-medium transition-colors">
                                            <span x-text="penguji2 === '{{ $dosen->id }}' ? 'Dipilih P2' : 'P2'"></span>
                                        </button>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="mt-3 rounded-lg bg-gray-50 px-4 py-3 text-xs space-y-1">
                    <div class="flex gap-2">
                        <span class="w-20 font-medium text-gray-500">Penguji 1:</span>
                        <span x-text="penguji1Nama || '— belum dipilih —'" :class="penguji1Nama ? 'font-semibold text-brand-700' : 'text-slate-400 italic'"></span>
                    </div>
                    <div class="flex gap-2">
                        <span class="w-20 font-medium text-gray-500">Penguji 2:</span>
                        <span x-text="penguji2Nama || '—'" :class="penguji2Nama ? 'font-semibold text-teal-700' : 'text-gray-400 italic'"></span>
                    </div>
                </div>
            ...
            {{-- Modal Setujui --}}
            <div x-show="modalSetujui" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
                ...
                <div @click.stop class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"
                    ...
                    <form method="POST" action="{{ route('kaprodi.akademik.sidang.setujui', $pengajuan) }}" class="space-y-3">
                        @csrf
            ...
            {{-- Modal Tolak --}}
            <div x-show="modalTolak" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
                 x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                <div @click.stop class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"
                     x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                    ...
                    <h3 class="mb-2 text-base font-bold text-slate-900">Tolak Pengajuan Sidang</h3>
                    <form method="POST" action="{{ route('kaprodi.akademik.sidang.tolak', $pengajuan) }}" class="space-y-4">
                        @csrf
                        ...
                        <div class="flex justify-end gap-3">
                            <button type="button" @click="modalTolak = false" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">Batal</button>
                            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 transition-colors">
                                <x-icon name="x-circle" class="h-4 w-4" />
                                Tolak
                            </button>
                        </div>
        ...
        @elseif ($pengajuan->status === 'disetujui')
            <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <p class="text-sm font-semibold text-emerald-800">Sidang Skripsi Disetujui</p>
                </div>
        ...
        {{-- Riwayat --}}
        <div class="rounded-xl border bg-white p-6 shadow-sm">
            <h3 class="mb-4 text-sm font-semibold text-gray-700">Riwayat Status</h3>
            <ol class="relative ml-2 space-y-4 border-l border-gray-200">
                @forelse ($pengajuan->statusHistories as $h)
                    <li class="ml-4">
                        ...
                        <p class="text-xs text-gray-400">{{ $h->created_at?->format('d M Y, H:i') }} — {{ $h->changedBy?->name }}</p>
                        <p class="text-sm font-medium text-gray-700">→ <span class="font-semibold">{{ $h->status_baru }}</span></p>
                        @if ($h->catatan) <p class="mt-0.5 text-xs text-gray-500">{{ $h->catatan }}</p> @endif
                    </li>
                @empty
                    <li class="ml-4 text-sm text-gray-400">Belum ada riwayat.</li>
                @endforelse
            </ol>
        </div>
    </div>
